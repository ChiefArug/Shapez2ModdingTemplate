...
# Check if we are on windows (if we are then we need to adjust the path)
...
PREVIEW_IMG=$CURRENT_DIR/Steam/image.png
...
# Execute
steamcmd +login "$(cat .steamuser)" +workshop_build_item "$TMP_VDF" +quit;
